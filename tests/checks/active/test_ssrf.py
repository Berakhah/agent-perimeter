...
def test_probe_refuses_without_a_scope_file() -> None:
    with pytest.raises(AuthorizationRequired):
        CHECK.run(_context(ProbeTransport("Connection refused to 127.0.0.1:9"), scope=None))


def test_assert_authorised_names_the_check() -> None:
    context = _context(ProbeTransport("Connection refused to 127.0.0.1:9"), scope=None)
    with pytest.raises(AuthorizationRequired, match="active.ssrf"):
        assert_authorised(context, "active.ssrf")
